defmodule CAIBot.Commands.PlanetSide.Stats do
  ...
  def command(message, [character_name, weapon_name]) do
    ...
    with weapon_name_matched when not is_nil(weapon_name_matched) <-
           Enum.find(
             CAIData.API.get_info(:weapon),
             &(String.replace(&1, [" ", "-"], "")
               |> String.downcase()
               |> String.contains?(weapon_name_simplified))
           ),
      ...
      base_embed =
        ...
        |> Embed.put_footer(
          "#{faction_name} weapon - To view weapon properties like mag size, reload time, etc. use !weapon",
          faction_logo
        )

      embed =
        Enum.reduce(weapon_stats, base_embed, fn {weapon, weapon_stats}, cur_embed ->
      ...
      nil ->
        Api.create_message!(message.channel_id, "No weapon by that name.")
      ...
      {:ok, %QueryResult{data: %{"name" => %{"first" => name}}}} ->
        Api.create_message(
          message.channel_id,
          "No #{weapon_name} stats for #{name} (Insufficient data.)"
        )

      {:error, error} ->
        Api.create_message(
          message.channel_id,
          "An error occurred while fetching the character/weapon. Please try again in a bit (and make sure both names are spelled correctly.)"
        )

        Logger.error(
          "Query Error for command !stats #{character_name} #{weapon_name}: #{inspect(error)}"
        )

      weapon_stats when weapon_stats == %{} ->
        Api.create_message(
          message.channel_id,
          "No #{weapon_name} stats for #{character_name} (Insufficient data.)"
        )
    end
  end
    ...
    with query <- char_stats_query(character_name),
          ...
          %QueryResult{
            data:
              %{
                ...
                "lt_stats" => lifetime_stats,
                "f_stats" => faction_stats
              } = character
          }} <- PS2.API.query_one(query),
         character_stats when character_stats != %{} <-
           build_char_stats(
             lifetime_stats,
             faction_stats,
             Map.get(character, "shot_stats", []),
             Map.get(character, "weapon_shot_stats", []),
             Map.get(character, "weapon_f_stats", [])
           ) do
      ...
      character_stats when character_stats == %{} ->
        Api.create_message(
          message.channel_id,
          "No stats for #{character_name} (Insufficient data.)"
        )
    end
  end
  ...
  defp char_stats_desc(base_stats, %{"times" => times}) do
    ...
    **--==+==--**
    **Created**: #{
      times["creation"] |> String.to_integer() |> DateTime.from_unix!() |> DateTime.to_date()
    }
    **Time In Game**: #{safe_div(String.to_integer(times["minutes_played"]), 60)} hours
    **Last Seen**: #{
      times["last_save"] |> String.to_integer() |> DateTime.from_unix!() |> DateTime.to_date()
    }
    """
  ...
  defp char_stats_query(character_name) do
    ...
    |> join(
      Join.new(collection: "characters_stat")
      |> list(true)
  ...
  defp build_char_stats(
    ...
    |> Enum.reduce(%{}, fn
      ...
      acc ->
        value =
          String.to_integer(value_nc) + String.to_integer(value_tr) + String.to_integer(value_vs)
      ...
      %{
        "stat_name" => stat_name,
        "value_forever_nc" => value_nc,
        "value_forever_tr" => value_tr,
        "value_forever_vs" => value_vs
      },
      acc ->
        Map.put(
          acc,
  ...
  defp build_weapon_stats(weapon_stats, weapon_faction_stats) do
    ...
    # Build weapon stats in the form of %{weapon name => %{stat name => value, ...} ...}
    |> Enum.reduce(%{}, fn
      %{"stat_name" => stat_name, "value" => val, "weapon" => %{"name" => %{"en" => w_name}}},
      acc ->
        value = String.to_integer(val)
    ...
    # Filter out "Anniversary Edition" weapons - may want to concat their stats onto the default version of the weapon later.
    |> Enum.filter(fn
      {weapon, stats} ->
