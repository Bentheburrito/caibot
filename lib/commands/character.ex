	...
	def command(message, character_name) do
		...
		case PS2.API.query_one(query) do
			...
			{:ok, %QueryResult{data: %{"name" => %{"first" => name}, "times" => times, "certs" => certs, "online_status" => online_status} = character}} ->
				...
				{faction_name, faction_color, faction_logo} = CAIData.API.get_info(:faction)[character["faction_id"]]

				outfit_tag = character["outfit"]["alias"]
				hours_played = times["minutes_played"] |> String.to_integer() |> div(60)
				cur_session_login = case CAIData.API.get_session(character["character_id"]) do
				...
				{footer, footer_img} =
					if online_status == "0" do
						{"Offline - Last seen #{times["last_save"] |> String.to_integer() |> DateTime.from_unix!() |> DateTime.to_date()}",
						"https://i.imgur.com/KenvqDV.png"}
					else
						{"Online for #{Utils.time_since_epoch(cur_session_login)}" <> " - #{character["character_id"]}",
						"https://i.imgur.com/hxZ9HC4.png"}
					end

				embed =
					%Embed{}
					|> Embed.put_title(["[#{outfit_tag}]", character["title"]["name"]["en"], name] |> Enum.filter(& not is_nil(&1) and &1 != "[]") |> Enum.join(" "))
					|> Embed.put_description([CAIData.API.get_info(:world)[character["world_id"]], faction_name, "[#{outfit_tag}] #{Map.get(character, "outfit")["name"]}" |> String.trim] |> Enum.filter(& not is_nil(&1) and &1 != "[]") |> Enum.join(", "))
