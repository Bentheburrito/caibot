defmodule CAIBot.Commands.PlanetSide.Session do
  ...
  def command(message, character_name) do
    case CAIData.API.get_session_by_name(character_name) do
      :none ->
        Api.create_message!(
          message.channel_id,
          "We don't have any session data for that character."
        )
      ...
      {:ok, session} ->
        ...
        top_xp_sources =
          if map_size(session.xp_types) > 0 do
            Enum.sort(session.xp_types, fn {_xp_id_1, value_1}, {_xp_id_2, value_2} ->
              value_1 >= value_2
            end)
            ...
            |> Enum.map_join("\n", fn {xp_id, value} ->
              ...
              "#{value}xp | #{safe_div(value, Map.get(xp_info, "xp")) |> ceil()}x | #{
                xp_info["description"]
              }"
            end)
          else
  ...
  defp general_stats_desc(session) do
    ...
    vehicles_destroyed_card =
      Enum.map_join(session.vehicles_destroyed, "\n", fn {name, amount} ->
        "#{amount}x #{name}"
      end)

    vehicles_lost_card =
      Enum.map_join(session.vehicles_lost, "\n", fn {name, amount} -> "#{amount}x #{name}" end)
  ...
  defp ivi_stats_desc(session) do
    ...
    **Score**: #{
      (session.archived == true && Float.round(hsr * accuracy, 2)) || "Pending API update..."
    }
    **--==+==--**
    ...
    **KPM**: #{
      safe_div(session.kills_ivi * 60, session.logout_timestamp - session.login_timestamp)
    }
    **--==+==--**
    **SPK**: #{
      (session.archived == true && safe_div(session.shots_fired, session.kills_ivi)) ||
        "Pending API update..."
    }
    **HPK**: #{
      (session.archived == true && safe_div(session.shots_hit, session.kills_ivi)) ||
        "Pending API update..."
    }
    """
  end
