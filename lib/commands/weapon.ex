defmodule CAIBot.Commands.PlanetSide.Weapon do
  ...
  def command(message, weapon_name) do
    ...
    with {:ok, %QueryResult{data: weapon_list, returned: returned}} when returned > 0 <-
           PS2.API.query(query) do
      ...
      if not is_nil(weapon_stats) do
        embed =
          ...
          |> Embed.put_field(
            "Stats",
            weapon_stat_desc(Map.get(weapon_stats, "weapon", %{})),
            true
          )
      ...
      {:error, error} ->
        Api.create_message(
          message.channel_id,
          "An error occurred while fetching the weapon. Please try again in a bit (and make sure the name is spelled correctly.)"
        )
  ...
  defp weapon_stat_desc(weapon) do
    ...
    """
    **Damage (min, max)**: #{Map.get(weapon, "damage", "N/A")} (#{weapon["damage_min"]}, #{
      weapon["damage_max"]
    })
  ...
  defp clarify_user_choice(message, weapons) do
    ...
    {embed, weapon_selection_map, _emotes} =
      Enum.reduce_while(weapons, {init_embed, %{}, CAIBot.reaction_map()}, fn
        %{"name" => %{"en" => name}, "description" => %{"en" => desc}} = weapon,
        {embed, weapon_selection_map, [reaction | emotes]} ->
          {:cont,
           {Embed.put_field(embed, "#{reaction} for #{name}", String.split(desc, ".") |> hd()),
            Map.put(weapon_selection_map, reaction, weapon), emotes}}

        _weapon, {_embed, _weapon_selection_map, []} = acc ->
          {:halt, acc}
      end)
    ...
    case CAIBot.ReactionHandler.await_reaction(prompt.id, users: [message.author.id]) do
      ...
      :timeout ->
        Api.create_message!(
          message.channel_id,
          "Cancelling !weapon command, user ran out of time to react."
        )
