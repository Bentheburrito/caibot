defmodule CAIBot.ReactionHandler do
  ...
  * `:timeout` - a timeout in milliseconds, or :infinity (defaults to #{@default_timeout}).
  ...
  def handle_call({:await, message_id, options}, from, awaiting) do
    ...
    if timeout != :infinity,
      do: Process.send_after(self(), {:timeout, {message_id, from}}, timeout)

    {:noreply, Map.put(awaiting, message_id, {filters, from})}
  end

  def handle_cast({:new_reaction, %{message_id: message_id} = reaction}, awaiting)
      when is_map_key(awaiting, message_id) do
  ...
  def handle_info({:timeout, {message_id, from}}, awaiting)
      when is_map_key(awaiting, message_id) do
